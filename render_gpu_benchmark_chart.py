...
def main() -> None:
    ...
    parts = [
        ...
        '<text x="80" y="85" fill="#f8fafc" font-family="Arial" font-size="46" font-weight="700">DirectML placement verified. CPU latency was lower.</text>',
        '<text x="80" y="135" fill="#94a3b8" font-family="Arial" font-size="25">Rice LSTM · warmed median latency · RX 6600 DirectML vs CPU · linear scale from zero · lower is better</text>',
    ...
    for index, row in enumerate(rows):
        ...
        parts.append(f'<text x="{center:.1f}" y="{top + chart_h + 75}" text-anchor="middle" fill="#67e8f9" font-family="Arial" font-size="19">DML ÷ CPU latency {cpu_advantage:.2f}×</text>')
    parts.extend(
        [
            '<text x="80" y="805" fill="#a7f3d0" font-family="Arial" font-size="24" font-weight="700">Profile proof: 15 / 18 node events on DirectML, including 2 LSTM events · max session-output difference 3.81e-6</text>',
